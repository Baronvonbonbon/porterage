...
export async function publishStatement(
  ...
  await allowance;
  const statement = { topics: [topic], channel, expiry: expiryIn(ttlSeconds), data: hexlify(bytes) };
...
export function publishRequest(bytes: Uint8Array, kind: "fund" | "payout" = "fund"): Promise<void> {
...
export async function subscribeTopics(
  ...
  const store = await getStatementStore();
  if (!store) throw new Error("the Polkadot app offers no Statement Store here");
  const sub = store.subscribe({ matchAny: topics as `0x${string}`[] }, (page) => {
    for (const s of (page as { statements: { data?: string; topics?: string[] }[] }).statements) {
      if (!s.data) continue;
      try {
