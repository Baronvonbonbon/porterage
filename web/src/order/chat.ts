...
import { rememberSaid, threadOf } from "../shield/notes";
...
/** Room for the order id, then 5 bytes of header per message. */
const HEAD = 8;
const PER_MESSAGE = 5;
...
export function decodeThread(
  bytes: Uint8Array
): { orderId: bigint; said: { at: number; text: string }[] } | null {
...
export async function say(
  ...
  await publishStatement(
    ...
    await seal(theirs, CHAT, encodeThread(orderId, said))
  );
  return said;
}
 ...
 * order — the other side as it arrives, this side from storage.
 */
export async function watchThread(
  ...
  const show = async () => {
    ...
    heard(
      [...ours, ...theirSide.map((m) => ({ ...m, mine: false }))].sort(
        (a, b) => a.at - b.at
      )
    );
  };
