...
import { SigningKey, Wallet, concat, getBytes, hexlify, keccak256 } from "ethers";
...
export async function seal(theirs: string, kind: number, payload: Uint8Array): Promise<Uint8Array> {
  const ephemeral = Wallet.createRandom();
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const body = new Uint8Array(
    await crypto.subtle.encrypt({ name: "AES-GCM", iv }, await sharedKey(ephemeral.signingKey, theirs), payload as BufferSource),
  );
...
export async function open(mine: Reader, kind: number, bytes: Uint8Array): Promise<Uint8Array | null> {
  if (bytes.length <= HEADER || bytes[0] !== VERSION || bytes[1] !== kind) return null;
  try {
