...
export const PAS_LOCATION: Location = { parents: 1, interior: { type: "Here", value: undefined } };
...
export async function quote(from: Location, to: Location, amountIn: bigint): Promise<bigint | null> {
  const out = (await api().apis.AssetConversionApi.quote_price_exact_tokens_for_tokens(
    from, to, amountIn, true,
  )) as bigint | undefined;
  return out && out > 0n ? out : null;
}

/** What it costs in `from` to buy exactly `amountOut` of `to`. */
export async function quoteForExact(from: Location, to: Location, amountOut: bigint): Promise<bigint | null> {
  const out = (await api().apis.AssetConversionApi.quote_price_tokens_for_exact_tokens(
    from, to, amountOut, true,
  )) as bigint | undefined;
  return out && out > 0n ? out : null;
}

/** PAS (planck) for `amount` of a token, and the other way round. */
export const quoteToPas = (token: Token, amount: bigint) => quote(locationOf(token.id), PAS_LOCATION, amount);
export const quoteFromPas = (token: Token, planck: bigint) => quote(PAS_LOCATION, locationOf(token.id), planck);
...
export async function planSwap(
  ...
  if (quoted === null) throw new Error("that pair has no liquidity on Asset Hub right now");
  return { from, to, amountIn, quoted, minOut: (quoted * (10_000n - slippageBps)) / 10_000n };
}
...
export async function tokenBalance(token: Token, address: string): Promise<bigint> {
  const acct = (await api().query.Assets.Account.getValue(token.id, address)) as { balance: bigint } | undefined;
...
export async function minBalance(token: Token): Promise<bigint> {
  const asset = (await api().query.Assets.Asset.getValue(token.id)) as { min_balance: bigint } | undefined;
 ...
 */
export async function spendableToken(token: Token, address: string): Promise<bigint> {
  const [balance, min] = await Promise.all([tokenBalance(token, address), minBalance(token)]);
