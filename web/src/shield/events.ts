...
const hex = (v: unknown): string =>
  typeof v === "string"
    ...
    ? hexlify(v)
    : hexlify((v as { asBytes(): Uint8Array }).asBytes());
...
interface EventRecord {
  event: {
    ...
    value: {
      type: string;
      value: { contract: unknown; data: unknown; topics: unknown[] };
    };
  };
}
...
export function contractEvents(
  ...
  return async (block) => {
    ...
    const events = (await client
      .getUnsafeApi()
      .query.System.Events.getValue({ at: hash })) as EventRecord[];
    const out: { topics: string[]; data: string }[] = [];
    for (const { event } of events) {
      if (event.type !== "Revive" || event.value.type !== "ContractEmitted")
        continue;
  ...
  return async (block) => {
    ...
    for (const { event } of events) {
      if (event.type !== "Revive" || event.value.type !== "ContractEmitted")
        continue;
      const e = event.value.value;
      if (hex(e.contract) !== want || !topics.includes(hex(e.topics[0])))
        continue;
