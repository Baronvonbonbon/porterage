...
import { BN254_R, POOL, b32, commitmentOf, notePathsAt, reconstructPath, type Note } from "../src/shield/pool";
...
const PAS_LOCATION = { parents: 1, interior: { type: "Here", value: undefined } };
const locationOf = (id: number) => ({
  parents: 0,
  interior: { type: "X2", value: [{ type: "PalletInstance", value: 50 }, { type: "GeneralIndex", value: BigInt(id) }] },
});
const quote = async (from: unknown, to: unknown, amt: bigint) =>
  (await api.apis.AssetConversionApi.quote_price_exact_tokens_for_tokens(from, to, amt, true)) as bigint | undefined;
const submit = async (call: unknown, what: string) => {
  const r = await (call as { signAndSubmit: (s: unknown, o: unknown) => Promise<{ ok: boolean; block: { number: number }; dispatchError?: unknown }> })
    .signAndSubmit(signer, { customSignedExtensions: HUB_EXTENSIONS });
  if (!r.ok) throw new Error(`${what} failed: ${JSON.stringify(r.dispatchError)}`);
...
for (const t of TOKENS) {
  ...
  console.log(`   ${t.symbol.padEnd(5)} ${out ? formatUnits(out, t.decimals, 4) : "no liquidity"}`);
}
...
const balance = ((await api.query.Assets.Account.getValue(usdc.id, me)) as { balance: bigint } | undefined)?.balance ?? 0n;
const min = ((await api.query.Assets.Asset.getValue(usdc.id)) as { min_balance: bigint }).min_balance;
...
console.log(`3. ${formatUnits(held, usdc.decimals, 4)} USDC quotes at ${Number(quoted) / 1e10} PAS → notes ${rungs.map((r) => r / 10n ** 18n).join(" + ")}`);
const field = () => (toBigInt(randomBytes(31)) % BN254_R).toString();
const notes: Note[] = rungs.map((r) => ({ nullifier: field(), secret: field(), value: r.toString(), asset: "0" }));
const commitments = notes.map(commitmentOf);
const swapBack = await api.tx.AssetConversion.swap_exact_tokens_for_tokens({
  path: [locationOf(usdc.id), PAS_LOCATION], amount_in: held, amount_out_min: minOut, send_to: me, keep_alive: true,
});
const deposits = [];
for (const [i, n] of notes.entries()) {
  const data = POOL.encodeFunctionData("depositNative", [b32(commitments[i])]);
  const value = BigInt(n.value) / 10n ** 8n;
  const dry: any = await api.apis.ReviveApi.call(me, SHIELD_POOL, value, undefined, undefined, getBytes(data));
  const w = dry.weight_required;
  deposits.push(api.tx.Revive.call({
...
const block = await submit(api.tx.Utility.batch_all({ calls: [swapBack.decodedCall, ...deposits] }), "the swap-and-shield batch");
...
for (const [i, n] of notes.entries()) {
  const { root } = await reconstructPath(eth, SHIELD_POOL, n, paths[i], inserts);
  console.log(`4. note ${i} at leaf ${paths[i].index} reaches the live root ${root.slice(0, 12)}…`);
}
client.destroy();
process.exit(0);
