...
import { Contract, JsonRpcProvider, Wallet, formatEther, toBigInt } from "ethers";
import { CHAIN, SHIELD_POOL } from "../src/config";
import DEPLOYED from "../src/deployed.json";
import { BN254_R, commitmentOf, findLeafBlock, notePathsAt, reconstructPath, type Note } from "../src/shield/pool";
import { contractEvents, poolInserts } from "../src/shield/events";
import { INSERTED_TOPIC, noteLeaves, payoutCommitment, proveSpend } from "../src/shield/payout";
...
const payee = new Wallet(readFileSync(join(homedir(), ".config", "porterage", "deploy-key"), "utf8").trim(), eth);
...
const send = async (label: string, p: Promise<{ hash: string; wait: () => Promise<unknown> }>) => {
  const tx = await p;
...
await new Promise((r) => setTimeout(r, 8000));
console.log(`1. vault balance ${formatEther(await vault.balanceOf(payee.address))} PAS`);

const field = (s: string) => (toBigInt(s) % BN254_R).toString();
const note = { n: 0, bucket: PAS.toString(), nullifier: field(Wallet.createRandom().privateKey), secret: field(Wallet.createRandom().privateKey) };
await send("inserted the note", vault.insertShieldNote(PAS, payoutCommitment(note), {}));
await new Promise((r) => setTimeout(r, 8000));
...
// 3
const ksNote: Note = { nullifier: field(Wallet.createRandom().privateKey), secret: field(Wallet.createRandom().privateKey), value: PAS.toString(), asset: "0" };
const ksCommitment = commitmentOf(ksNote);
const leaves = await noteLeaves(eth, vaultAddr, insertedAt, contractEvents(client, vaultAddr, [INSERTED_TOPIC]));
let t = performance.now();
const shield = (f: string) => new Uint8Array(readFileSync(join(import.meta.dirname, "..", "public", "shield", f)));
const spend = await proveSpend(note, leaves, ksCommitment, { wasm: shield("shieldnote.wasm"), zkey: shield("shieldnote.zkey") });
console.log(`3. proved in ${Math.round(performance.now() - t)} ms over ${leaves.length} note(s)`);

// 4 — through the request format, submitted by a key that never held the note
await send("funded the stranger", payee.sendTransaction({ to: stranger.address, value: PAS / 2n }));
await new Promise((r) => setTimeout(r, 8000));
const words = Array.from({ length: 8 }, (_, i) => BigInt("0x" + spend.proof.slice(2).slice(64 * i, 64 * (i + 1))).toString());
const req = decodePayout(encodePayout({ bucket: PAS, root: spend.root, nullifierHash: spend.nullifierHash, ksCommitment: spend.ksCommitment, words }));
const out = await submitPayout(req, vaultAddr, stranger);
console.log(`4. stranger ${stranger.address}: ${out.status === "sent" ? `sent, gas ${out.gas}` : out.reason}`);
...
for (let i = 0; i < 30 && block === null; i++) {
  ...
  block = await findLeafBlock(eth, SHIELD_POOL, ksCommitment, insertedAt, inserts);
}
if (block === null) throw new Error("the pool note never appeared");
const [path] = await notePathsAt(eth, SHIELD_POOL, block, [ksCommitment], inserts);
const { root } = await reconstructPath(eth, SHIELD_POOL, ksNote, path, inserts);
console.log(`5. pool note at leaf ${path.index} (block ${block}) reaches the live root ${root.slice(0, 12)}…`);
client.destroy();
process.exit(0);
