...
import { POOL, BN254_R, b32, commitmentOf, notePathsAt, reconstructPath, type Note } from "../src/shield/pool";
...
const words = readFileSync(process.env.KEY_FILE ?? join(homedir(), ".config", "porterage", "name-key"), "utf8").trim();
...
const notes: Note[] = [1n, 1n].map((p) => ({ nullifier: field(), secret: field(), value: (p * 10n ** 18n).toString(), asset: "0" }));
...
for (const [i, n] of notes.entries()) {
  ...
  if (!dry.result.success || dry.result.value.flags & 1) throw new Error(`dry run: ${JSON.stringify(dry.result, (_k, v) => (typeof v === "bigint" ? v.toString() : v))}`);
  const w = dry.weight_required;
  const dep = dry.storage_deposit.type === "Charge" ? dry.storage_deposit.value : 0n;
  inner.push(api.tx.Revive.call({ dest: SHIELD_POOL, value, weight_limit: { ref_time: (w.ref_time * 6n) / 5n, proof_size: (w.proof_size * 6n) / 5n }, storage_deposit_limit: (dep * 6n) / 5n, data: getBytes(data) }).decodedCall);
}
console.log(`depositing 2 × 1 PAS from ${origin}`);
const r = await api.tx.Utility.batch_all({ calls: inner }).signAndSubmit(signer, { customSignedExtensions: HUB_EXTENSIONS });
...
const paths = await notePathsAt(eth, SHIELD_POOL, r.block.number, commitments, inserts);
console.log("indexes", paths.map((p) => p.index));
for (const [i, n] of notes.entries()) {
